...
export default function CreateAccountForm() {
  ...
  const handleSubmit = useCallback(async (event: FormEvent) => {
    ...
    if ((!nameRef.current?.validity.valid) || (!balanceRef.current?.validity.valid)) {
  ...
  return (
    <form method="POST" noValidate className={`form ${wasValidated ? "was-validated" : ""}`} onSubmit={handleSubmit}>
      ...
      <div className="form__row">
        ...
        <select name="currency" id="currency" value={currency} onChange={(event) => setCurrency(event.target.value)}>
